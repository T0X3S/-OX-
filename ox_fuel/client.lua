...
local function startFueling(vehicle, isPump)
	ESX.TriggerServerCallback('ox_fuel:getBankMoney', function(bankMoney)
	...
	if isPump then
		...
		if Config.priceTick > moneyAmount then
			if Config.priceTick > bankMoney then
				return lib.notify({
					type = 'error',
			...
			options = {
				{
					action = function (entity)
						if ox_inventory:Search(2, 'money') >= Config.priceTick then
							...
							ESX.TriggerServerCallback('ox_fuel:getBankMoney', function(bankMoney)
								if bankMoney >= Config.priceTick then
									startFueling(lastVehicle, 1)
								else
				...
				},
				{
					action = function (entity)
						...
						ESX.TriggerServerCallback('ox_fuel:getBankMoney', function(bankMoney)
							...
							if moneyAmount < Config.petrolCan.price then
								if bankMoney < Config.petrolCan.price then
									notEnoughMoney = true
									return lib.notify({type = 'error', description = locale('petrolcan_cannot_afford')})
								end
							end

							if not notEnoughMoney then
